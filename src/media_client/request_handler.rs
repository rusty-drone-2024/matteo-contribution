...
impl MediaClient {
    ...
    fn fetch_homepage_data(&self) {
        let _ = self.network_request.send(ClientNetworkRequest::ListAll(self.counter));
        println!("  --> ASK ALL");
    }

    fn fetch_page_data(&self, node_id: NodeId, url: Link) {
        let _ = self.network_request.send(
            ClientNetworkRequest::Get(self.counter, node_id, url.clone())
        );
        println!("  --> ASK {}", url);
    }
}
